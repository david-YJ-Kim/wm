...
start()
{
    ...
    if [ -n "${PID}" ]; then
        ...
        echo "/usr/lib/java/openjdk-8u262-b10/bin/java -jar -Dspring.config.additional-location=${CONF} -Dspring.profiles.active=${PROFILE} -Dname=${PROC_NAME} -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=./ -Xms1024m -Xmx4096m -XX:MaxMetaspaceSize=256m -XX:MetaspaceSize=128m -XX:+UseG1GC -Dcom.sun.management.jmxremote.port=15021 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false "${DAEMON}""
        nohup /usr/lib/java/openjdk-8u262-b10/bin/java -jar -Dspring.config.additional-location=${CONF} -Dspring.profiles.active=${PROFILE} -Dname=${PROC_NAME} -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=./ -Xms1024m -Xmx4096m -XX:MaxMetaspaceSize=256m -XX:MetaspaceSize=128m -XX:+UseG1GC -Dcom.sun.management.jmxremote.port=15021 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false ${DAEMON} > /dev/null 2>&1 &
        local PID=${!}
